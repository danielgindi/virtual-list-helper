import { getElementOffset } from '@danielgindi/dom-utils/lib/Css';
...
class VirtualListHelper {
  ...
  createGhostItemElement(ghostIndex, append, ghostTester) {
    ...
    try {
      if (append) {
        (p.virtualWrapper || p.list).appendChild(itemEl);
      }
      ghostTester(itemEl);
    } finally {
      if (append) {
  ...
  _dequeueElementForIndex(itemEl, index, insertBefore) {
    ...
    const itemParent = virtualWrapper || list;
    const existingEls = p.existingEls;
 ...
 * @returns {number}
 */
let binarySearchPosition = function (positions, pos, start = 0, end = -1) {
 ...
 * @returns {number}
 */
let binarySearchPositionByFn = function (total, fn, pos, start = 0, end = -1) {
  if (end < 0)
    end += total;
  if (end <= start) return end; // 0 or 1 length array
 ...
 * @returns {undefined|*}
 */
let findLast = function (array, fn) {
